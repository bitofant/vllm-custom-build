...
- Gemma 4 MTP (Multi-Token Prediction) speculative decoding uses Google's drafter `google/gemma-4-31B-it-assistant` (~0.5B), enabled by `vllm.sh` config **`4m`** (same NVFP4 checkpoint as `4`, but with `--speculative-config` and context trimmed so the drafter weights fit the KV budget at gpu-mem-util 0.983). **Context ceiling is not fixed — it tracks the measured KV pool, which shrinks whenever a vLLM bump grows the non-KV footprint.** History: `4m` ran **128k** until the 2026-07-17 vLLM update (Qwen 3.6 support + new default kernels/cudagraphs) dropped the KV pool to ~6.86 GiB (~85k tokens), at which point 128k OOMed the KV cache at startup ("KV cache is needed, which is larger than the available KV cache memory"). Re-tuned **2026-07-17 to `CONTEXT_SIZE=83968`** (block-aligned to 16, ~1.02x concurrency — the max single-request context that boots with a thin margin for profiling jitter; validated serving end-to-end). This is a `vllm.sh`-only fix (no image rebuild). **When re-tuning after a base/source bump, re-derive from the engine's `GPU KV cache size: <N> tokens` log line and set `CONTEXT_SIZE` just under `N`.**
